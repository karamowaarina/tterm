    ...
    internal static class Program
    {
        ...
        static void Main()
        {
            ...
            mainForm.AddFilter(new PixelFilter<HueParameters>(
                ...
                (pixel, parameters) =>
                {
                    var hNew = pixel.H + parameters.Coefficient;
                    if (hNew > 360)
                        hNew -= 360;
                    return Convertors.HSLToPixel(hNew, pixel.S, pixel.L);
                }));
            
            mainForm.AddFilter(new MosaicTransformFilter(
                "Мозаика",
                new MosaicTransformer()
            ));

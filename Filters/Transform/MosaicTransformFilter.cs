...
{
    public class MosaicTransformFilter : ParametrizedFilter<EmptyParameters>
    {
        MosaicTransformer transformer;

        public MosaicTransformFilter(string name, 
            MosaicTransformer transformer)
        {
            this.name = name;
        ...
        public override Photo Process(Photo original, EmptyParameters parameters)
        {
            ...
            for(var y= 0; y < result.Height; y++)
            {
                for (var i = 0; i < 4; i++)
                {
                    var oldPoint = transformer.MapPoint(new Point(x, y), i);
